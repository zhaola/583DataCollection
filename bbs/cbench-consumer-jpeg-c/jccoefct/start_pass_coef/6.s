	.text
	.file	"jccoefct.c"
	.globl	start_pass_coef.6       # -- Begin function start_pass_coef.6
	.p2align	4, 0x90
	.type	start_pass_coef.6,@function
start_pass_coef.6:                      # @start_pass_coef.6
	.cfi_startproc
# %bb.0:                                # %newFuncRoot
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset %rbp, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register %rbp
	jmp	.LBB0_2
.LBB0_1:                                # %"11.exitStub"
	popq	%rbp
	.cfi_def_cfa %rsp, 8
	retq
.LBB0_2:                                # %"6"
	.cfi_def_cfa %rbp, 16
	movq	__profc_.._jccoefct.c_start_pass_coef+8, %rax
	addq	$1, %rax
	movq	%rax, __profc_.._jccoefct.c_start_pass_coef+8
	movq	(%rdi), %rax
	movabsq	$compress_first_pass, %rcx
	movq	%rcx, 8(%rax)
	jmp	.LBB0_1
.Lfunc_end0:
	.size	start_pass_coef.6, .Lfunc_end0-start_pass_coef.6
	.cfi_endproc
                                        # -- End function
	.hidden	__profc_.._jccoefct.c_start_pass_coef
	.hidden	compress_first_pass
	.ident	"clang version 10.0.0 "
	.section	".note.GNU-stack","",@progbits
